...
- **Phase 1 R6 (area context fan-out)**: live. `server/api/atlas/area-context.post.ts` (Nitro endpoint) calls the Elemental gateway via `/elemental/find` (linked-to-area for articles + economic concepts) and `/elemental/entities/properties` (article titles), and the panel renders the actual articles and concepts when an area is pinned. **Events** (PRD R6.1 step 1) are not exposed via the REST gateway — they require the MCP tool `elemental_get_events`; the panel surfaces a placeholder note and the JSON shape is final, only the implementation is stubbed. Per-retailer events (R6.1 step 3) await the registry adding `cik` / `org_neid` per retailer.
...
| Area context (R6)         | `composables/useAreaContext.ts`, `server/api/atlas/area-context.post.ts`                                                    | Nitro endpoint fans out a pinned area's NEID into linked articles + economic concepts via the Elemental gateway. Events stub awaits MCP transport.                 |
