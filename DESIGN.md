...
- **Phase 1 R6 (area context fan-out)**: stubbed. `composables/useAreaContext.ts` shapes the `{ area_events, area_articles, economic_concepts, retailer_events }` payload but the Nitro endpoint at `/api/atlas/area-context` is not yet wired. Per Phase 0 RED, the panel falls back to "context unavailable for this area" until area-NEIDs are resolved (Phase 1 backlog: run `expand-area-neids.ts` for the 3,601 areas).
...
| Area context (R6)         | `composables/useAreaContext.ts`                                                                                             | Stubbed Elemental fan-out (events / articles / economic concepts / retailer events). Nitro endpoint wiring is the next-step.                                       |
