...
- **R12 (documentation + handoff)**: shipped. [`README.md`](README.md) covers architecture + dev loop + deploy steps. [`DATA.md`](DATA.md) is the source-by-source catalog (CSVs, topojson, NEID caches, probe artifacts). [`COVERAGE.md`](COVERAGE.md) is auto-regenerated by [`scripts/build-coverage.ts`](scripts/build-coverage.ts) (`npm run build:coverage`) from the existing report JSONs — area + retailer NEID resolution rates, per-retailer halo coverage, latest Phase-0 store-NEID probe result, known gaps. [`design/RECIPES.md`](design/RECIPES.md) was already in place.
- **Phase 3 R4 (multi-retailer overlays)**: shipped. When `recipe === 'none'` and ≥2 retailers are active, the polygon's solid choropleth fill comes from the **lead retailer** (first active in `activeRetailers`, color from the registry's `hsl(...)`). Each non-lead retailer renders as an SVG `<pattern>` overlay on top — kinds cycle through hatch-45 → hatch-135 → dots → vertical, opacity = `sqrt(retailer_count / max_retailer_count)` per area. Hover tooltip surfaces the per-retailer breakdown for that polygon regardless of which retailer drew the fill. Legend's "Active retailers" rows now render the matching swatch (solid for lead, mini-pattern for the rest).
- **R6.3 / R10 (KV cache + telemetry)**: shipped. `server/utils/atlasKv.ts` adds a `withKvCache` read-through helper (1 h TTL for area-context + event-density, 6 h for opens-closes) wrapping all three Nitro fan-out routes; degrades gracefully when KV is unconfigured. The same util exposes `logToolCalls` (capped Upstash list of the most recent 10k calls) consumed by `GET /api/atlas/telemetry/summary?hours=N` for ops aggregations. The footer strip surfaces a session-level cache-hit-rate from the new `useAtlasTelemetry` composable. See [`ROADMAP.md`](ROADMAP.md) R-001 / R-007.
- **Phase 3 R7 (analysis recipes)**: shipped — three canonical recipes plus URL state serialization. R7.3 (cross-retailer co-occurrence) is pure client arithmetic with a top-25 ranking table. R7.1 (event-density choropleth) fans out top-K=100 areas via the MCP `elemental_get_events` tool in ~7 s. R7.2 (opens-vs-closes delta) classifies each retailer-level event as +1/−1 via category exact-match + keyword regex fallback, attributes via zero-padded participant NEIDs against `areas.json`. The graph doesn't carry discrete Store-Opening/Closure categories, so R7.2 is sparse but trustworthy — see [`design/RECIPES.md`](design/RECIPES.md) for the honest-signal caveats. R7.4 URL sync (`?c=&r=&t=&f=&halo=&p=`) is two-way bound; the rail's share button copies the current URL. Probe artifacts in `data/probes/event-categories-*.{json,md}`.
...
- **Phase 1 R9.1 (auth + access)**: shipped. Aether's Auth0 wiring covers the magic-link + cookie auth half of R9.1; the PRD's "anonymous → marketing splash + access request form" half is now [`pages/welcome.vue`](pages/welcome.vue) (the new public landing surface, watchdog-allowed). Submissions POST to [`server/api/atlas/access-request.post.ts`](server/api/atlas/access-request.post.ts), which writes to the Upstash list `atlas:access-requests:v1` (capped 1k); admin viewer at [`pages/admin/access-requests.vue`](pages/admin/access-requests.vue). The PRD's "hardcoded allowlist in env" gate lives in [`server/utils/atlasAllowlist.ts`](server/utils/atlasAllowlist.ts) — `assertAllowlistedUser(event)` reads `ATLAS_ALLOWLIST` (comma-separated emails) from runtime config and 403s any unsealed-cookie email not on the list. Wired into all atlas API routes (area-context, both recipes, telemetry summary, access-request list). When `ATLAS_ALLOWLIST` is empty, the gate is a no-op so local dev keeps working.
- **R-014 (access-request email notification)**: shipped. [`server/api/atlas/access-request.post.ts`](server/api/atlas/access-request.post.ts) now sends a best-effort Resend email after successful request persist via [`server/utils/accessRequestNotify.ts`](server/utils/accessRequestNotify.ts). Controlled by `RESEND_API_KEY` + `ATLAS_NOTIFY_EMAIL` (+ optional `ATLAS_NOTIFY_FROM`). Missing env or send failure downgrades to `notified: false` without breaking form submission.
- **R-006 (guarded text attribution, partial)**: shipped as a conservative increment. [`server/api/atlas/recipe/opens-closes.post.ts`](server/api/atlas/recipe/opens-closes.post.ts) still treats participant-NEID matches as the primary attribution path, but now adds a strict US-only text fallback when participants miss: explicit `"<area suffix>, <ST>"` mentions (e.g. `Fulton County, GA`) map to county keys. This avoids the prior false-positive class (`"McDonald's"` vs `"McDonald County"`), improves recall modestly, and keeps full NER as a deferred follow-up.
- **R-010 (hover tooltip upgrade)**: shipped. [`components/AtlasMapCanvas.vue`](components/AtlasMapCanvas.vue) moved from browser-native SVG `<title>` tooltips to a delegated Vue overlay (`v-card`) driven by one `@pointermove` listener on the SVG root. This removes per-polygon hover listeners, keeps styling consistent cross-browser, and leaves a clean extension point for richer hover content (chips/sparklines) without changing geometry rendering.
- **R-012 (client perf marks)**: shipped. [`composables/usePerfMarks.ts`](composables/usePerfMarks.ts) records TTFR, TTFI, and panel-open-latency from `performance.now()`. `AtlasMapCanvas` emits first-render + first-interactive once per session; `AtlasContextPanel` records panel latency per area pin with outcome + cache-hit tags. Metrics forward to `window.posthog.capture('atlas_perf_metric', ...)` when a PostHog client is present and no-op otherwise.
- **R-015 (Neon substrate, Phase A)**: in progress. Added Neon utility + schema bootstrap + backfill/parity tooling: [`server/utils/neon.ts`](server/utils/neon.ts), [`server/api/db/atlas-setup.post.ts`](server/api/db/atlas-setup.post.ts), [`scripts/db-atlas-setup.ts`](scripts/db-atlas-setup.ts), [`scripts/load-neon-atlas.ts`](scripts/load-neon-atlas.ts), [`scripts/report-neon-parity.ts`](scripts/report-neon-parity.ts). Runtime reads remain JSON-first until the Phase A parity gate is run and a DB-read flag is introduced.
...
| Recipes (R7)              | `composables/useAtlasRecipe.ts`, `components/AtlasRankingTable.vue`, `server/api/atlas/recipe/*.post.ts`                    | Three analysis recipes (cross-retailer co-occurrence, event density, opens-vs-closes) wired through one composable + recipe-aware canvas/legend. See [`design/RECIPES.md`](design/RECIPES.md) for math + caveats.                                                                      |
| Recipe URL sync (R7.4)    | `composables/useAtlasUrlSync.ts`, `components/AtlasShareButton.vue`                                                         | Two-way bind atlas state to `?c=&r=&t=&f=&halo=&p=` via `router.replace`. Share button copies the current URL. Shareable links = product surface.                                                                                                                                      |
| Event-categories probe    | `scripts/probe-event-categories.ts`, `data/probes/event-categories-*.{json,md}`                                             | Step 3a of the R7 plan — sampled 100 events per retailer, tallied categories and open/close keyword hits. Sourced the OPEN_CATEGORIES / CLOSE_CATEGORIES lists for R7.2. Re-run when retailer roster changes.                                                                          |
